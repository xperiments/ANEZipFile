...
+ (NSMutableArray*)getFilesInZip:(NSString *)path password:(NSString *)password 
{
	...
	// Begin unzipping
	if (unzGoToFirstFile(zip) != UNZ_OK) { return NULL; }
	...
	do {
		...
		unzGetCurrentFileInfo(zip, &fileInfo, filename, fileInfo.size_filename + 1, NULL, 0, NULL, 0);
		filename[fileInfo.size_filename] = '\0';
        [outputArray addObject:[NSString stringWithUTF8String:filename] ];
...
+(NSMutableData*)unzipFileFrom:(NSString*)path withName:(NSString*)fileName
{
    return [ self unzipFileFrom:path withName:fileName password:nil];
}
...
+(NSMutableData*)unzipFileFrom:(NSString*)path withName:(NSString*)fileName password:(NSString*)password
{
    ...
    NSMutableData *buffer= [[[NSMutableData alloc] initWithLength:8192] autorelease];
    NSMutableData* bytearray = [[[NSMutableData alloc] init] autorelease];
    
    
    unzLocateFile (zip, [fileName UTF8String], 0);
    
    if ([password length] == 0) {
        unzOpenCurrentFile(zip);
    } else {
        unzOpenCurrentFilePassword(zip, [password cStringUsingEncoding:NSASCIIStringEncoding]);
    }
    

    
    
    SSZipArchiveReadStream *readStream = [[[SSZipArchiveReadStream alloc] initWithUnzFileStruct:zip fileNameInZip:fileName ] autorelease];
...
+ (BOOL)createZipFileAtPath:(NSString *)path withFilesAtPaths:(NSArray *)paths andDelegate:(id<SSZipArchiveDelegate>)delegate 
{
	...
	if ([zipArchive open]) {
        if([delegate respondsToSelector:@selector(zipArchiveWillZipArchiveAtPath:)])
        {
            ...
            [delegate zipArchiveWillZipArchiveAtPath:path];
        }
		...
		for (NSString *path in paths) {
            ...
            if([delegate respondsToSelector:@selector(zipArchiveDidProgress:on:)])
            {
                ...
                [delegate zipArchiveDidProgress:current on:total];
            }
            current ++;
		}
        ...
        if([delegate respondsToSelector:@selector(zipArchiveDidZipArchiveAtPath:withSuccess:)])
            ...
            [delegate zipArchiveDidZipArchiveAtPath:path withSuccess:success];
	}
...
+ (BOOL)createZipFileAtPath:(NSString *)path withFilesAtPaths:(NSArray *)paths atPaths:(NSArray*)atPaths andDelegate:(id<SSZipArchiveDelegate>)delegate
{
	...
	if ([zipArchive open]) {
        ...
        int current = 1;
		for (NSString *path in paths) {
			[zipArchive writeFileAtPath:path filePath:[atPaths objectAtIndex:current-1] ];
...
- (BOOL)writeFileAtPath:(NSString *)path filePath:(NSString*)filePath
{
	...
	zipOpenNewFileInZip(_zip, [filePath UTF8String], NULL, NULL, 0, NULL, 0, NULL, Z_DEFLATED,
...
+ (BOOL)isPasswordProtected:(NSString*)path
{
	...
	int err= unzGetCurrentFileInfo(zip, &file_info, filename_inzip, sizeof(filename_inzip), NULL, 0, NULL, 0);
	if (err != UNZ_OK) {
        return NO;
	}

    BOOL isEncripted = ((file_info.flag & 1) != 0);
    unzClose(zip);
    return isEncripted;
